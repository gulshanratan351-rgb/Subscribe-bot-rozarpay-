        ...
        if data.get('event') == 'payment_link.paid':
            ...
            if order:
                ...
                expiry = int(
                    (
                        datetime.now() +
                        timedelta(minutes=int(order['mins']))
                    ).timestamp()
                )
...
def deapprove_user(message):
    ...
    try:
        ...
        if target_id:
            ...
            bot.send_message(
                target_id,
                "❌ Your Prime membership has been revoked by Admin."
            )

            bot.reply_to(
                message,
                f"✅ User {target_id} ko Deapprove kar diya gaya hai."
            )
...
def short_link(message):
    ...
    msg = bot.reply_to(
        message,
        "🔗 Paste the original link to shorten:"
    )
    ...
    if match:
        ...
        if is_prime(uid):
            ...
            markup.add(
                ...
                InlineKeyboardButton(
                    "🔥 7 Days ₹99",
                    callback_data=f"pay_{fid}_10080_99"
                )
            )
            ...
            markup.add(
                InlineKeyboardButton(
                    "👑 1 Month ₹249",
                    callback_data=f"pay_{fid}_43200_249"
                ),

                InlineKeyboardButton(
                    "💎 3 Months ₹649",
                    callback_data=f"pay_{fid}_129600_649"
                )
            )
        ...
        if is_prime(uid):
            ...
            text += (
                f"👑 Status: Prime User\n"
                f"📅 Expiry: {get_expiry_date(u['expiry'])}"
            )
